...
class sphere:
    def __init__(self):
        self.radius = 10#random.uniform(4, 20)
        ...
        self.xbobrate = 0#random.uniform(1, 10)
        self.xbobamount = 0#random.uniform(0.1, 1)

        self.ybobrate = 0#random.uniform(1, 10)
        self.ybobamount = 0#random.uniform(0.1, 1)
...
for epoch in range(10):
    ...
    for r in rays:
        while r.hit == False:
            ...
            for s in sphs:
                dist = ((abs(r.x - s.p[0])**2 + abs(r.y - s.p[1])**2 + abs(r.z - s.p[2])**2 + s.w**2)**0.5) - s.radius
                dists.append(dist)
                dists.sort()
    ...
    img.save('imgs//{0}.png'.format(frame))


    print(px, py, pz)
    print(starthangle, startvangle)
    for s in sphs:
        print(s.p)
